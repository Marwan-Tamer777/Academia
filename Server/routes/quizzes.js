...
router.post("/", verifyToken.verifyTokenAndAuthorizedTeacher, asyncHandler(async (req, res) => {
    ...
    let course = await courseModel.courseModel.findOne({ id: requestQuiz.courseId });
    if (!course) {
        return res.status(404).json({ error: 'The course with the given ID was not found' });
    }

    // create quiz
    const quiz = new model.quizModel({
        id: requestQuiz.id,
        courseId: requestQuiz.courseId,
...
router.get("/", verifyToken.verifyTokenAndAdmin, asyncHandler(async (req, res) => {
    const quizzes = await model.quizModel.find().select('-_id');
...
router.get(`/:id`, verifyToken.verifyToken, asyncHandler(async (req, res) => {
    const quiz = await model.quizModel.findOne({ quizId: req.params.id }).select('-_id');
    if (!quiz) {
...
router.put(`/:id`, verifyToken.verifyTokenAndAuthorizedTeacher, asyncHandler(async (req, res) => {
    ...
    // update quiz
    const quiz = await model.quizModel.findOneAndUpdate({ id: req.params.id },
        {
            quizName: requestQuiz.quizName,
            questions: requestQuiz.questions,
            numOfRetries: requestQuiz.numOfRetries,
...
router.delete(`/:id`, verifyToken.verifyTokenAndAuthorizedTeacher, asyncHandler(async (req, res) => {
    const quiz = await model.quizModel.findOneAndDelete({ id: req.params.id }).select('-_id');
    if (!quiz) {
        return res.status(404).json({ error: 'The quiz with the given ID was not found' });
    }
    res.status(200).json(functions.responseBodyJSON(
