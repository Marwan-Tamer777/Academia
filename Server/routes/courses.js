...
/// Old Data
// const courses = [
//     { id: 1, name: 'course1', tutor: 'Tutor A', level: 'Intermediate', language: 'English' },
//     { id: 2, name: 'course2', tutor: 'Tutor B', level: 'Beginner', language: 'Spanish' },
//     { id: 3, name: 'course3', tutor: 'Tutor C', level: 'Advanced', language: 'French' },
// ];
...
router.get("/", asyncHandler(async (req, res) => {
    ...
    const courses = await model.courseModel.find().select('-_id');
...
router.get(`/:id`, asyncHandler(async (req, res) => {
    const course = await model.courseModel.findOne({ id: req.params.id }).select('-_id');
    if (!course) {
...
router.post("/", asyncHandler(async (req, res) => {
    ...
    const course = new model.courseModel({
        id: requestCourse.id,
    ...
    const { _id, ...data } = course._doc;//remove _id from response
    // save course
    await course.save();
...
router.put("/:id", verifyToken.verifyTokenAndAdmin, asyncHandler(async (req, res) => {
    ...
    // find the course
    const course = await model.courseModel.findOne({ id: req.params.id });
    if (!course) {
        return res.status(404).json({ error: 'The course with the given ID was not found' });
    }
    // update course
    const requestCourse = req.body.context.course
    const updatedCourse = await model.courseModel.findOneAndUpdate({ id: req.params.id }, {
        name: requestCourse.name,
        courseCode: requestCourse.courseCode,
        enrollCode: requestCourse.enrollCode,
        teachers: requestCourse.teachers,
    ...
    }).select('-_id');// remove _id from response
    // save course
...
router.delete("/:id", verifyToken.verifyTokenAndAdmin, asyncHandler(async (req, res) => {
    ...
    const course = await model.courseModel.findOneAndDelete({ id: req.params.id }).select('-_id');
    if (!course) {
        return res.status(404).json({ error: 'The course with the given ID was not found' });
    }
    res.status(200).json(functions.responseBodyJSON(
